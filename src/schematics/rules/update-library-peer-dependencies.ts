    ...
    for (const packageName in libraryPackageJson[section]) {
      ...
      if (!rootVersion) {
        context.logger.error(
          `The package "${packageName}" is listed in the peer dependencies section of '${libraryPackageJsonPath}' but was not found in the root 'package.json'!`
        );
        continue;
      }
...
export function updateLibraryPeerDependencies(): Rule {
